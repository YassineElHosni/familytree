/*  code source https://github.com/shenoyranjith/family-tree-generator */
import { fabric } from "fabric"

import imgUrl from "../assets/default_profile.png"
import { BaseNode, BaseRelation } from "../types/base.types"
...
export default class TreeGenerator {
    ...
    private nodeClickCallback: (node: Node) => void

    constructor(root: Node, options: Options, nodeClickCallback: (node: Node) => void) {
        this.root = root
    ...
    private _createCanvas = (options: Options) => {
        ...
        let height =
            parentEle != undefined && options.height > parentEle.clientHeight && options.boundToParentSize
                ? parentEle.clientHeight
                : options.height
        let width =
            parentEle != undefined && options.width > parentEle.clientWidth && options.boundToParentSize
    ...
    private _setupCanvas = () => {
        ...
        function touchZoom(this: Canvas, opt: any) {
            ...
            if (evt.touches && evt.touches.length == 2) {
                ...
                let point1 = new fabric.Point(evt.touches[0].clientX, evt.touches[0].clientY)
                let point2 = new fabric.Point(evt.touches[1].clientX, evt.touches[1].clientY)
        ...
        this.canvas.on("mouse:down", function (this: Canvas, opt) {
            ...
            let isTouch = evt.type === "touchstart" && (evt as TouchEvent).touches.length === 1
            this.isDragging = true
            this.setCursor("grabbing")
            this.lastPosX = isTouch ? (evt as TouchEvent).touches[0].clientX : (evt as MouseEvent).clientX
            this.lastPosY = isTouch ? (evt as TouchEvent).touches[0].clientY : (evt as MouseEvent).clientY
        })
        this.canvas.on("mouse:move", function (this: Canvas, opt) {
            if (this.isDragging) {
                let isTouch = opt.e.type === "touchmove"
                var evt = opt.e as MouseEvent | TouchEvent
                let clientX = isTouch ? (evt as TouchEvent).touches[0].clientX : (evt as MouseEvent).clientX
                let clientY = isTouch ? (evt as TouchEvent).touches[0].clientY : (evt as MouseEvent).clientY
    ...
    private _setImageSrc = async (imageObject: fabric.Image, imageUrl: string): Promise<fabric.Image> => {
    ...
    private _drawPartnerLine = (node1: fabric.Group, node2: fabric.Group, isMarried: boolean) => {
    ...
    private _drawParentLine = (parent: fabric.Line | Node, isFirst: boolean | undefined) => {
        ...
        if (parent instanceof fabric.Line) {
            ...
            if (!isFirst) {
                parentLineOrigin = parentLineOrigin.add(
                    new fabric.Point(((parent.x2 as number) - (parent.x1 as number)) / 2 - nodeRadius, 0)
                )
            }
        } else {
    ...
    private _drawChildLine = (child: Node, parentLine: fabric.Line) => {
        ...
        const horizontalLine = new fabric.Line(
            [
                (parentLine.x2 as number) + ((parentLine.x2 as number) > childCenter.x ? strokeWidth : 0),
    ...
    private _drawNode = async (node: Node) => {
        ...
        nodeObject.on("mousedown", () => {
            this.defautOnClick(node)
        })
        node._object = nodeObject
        const relationships = node.relationships

        nodeObject.set(position)
        ...
        if (relationships && relationships.length > 0) {
            ...
            for (const relationship of node.relationships) {
                if (relationship.partner) {
                    const parnterNode = await this._createNode(relationship.partner.name, relationship.partner.image)
                    parnterNode.on("mousedown", () => {
                        this.defautOnClick(relationship.partner)
                    })
                    relationship.partner._object = parnterNode
                    parnterNode.set(position)
    ...
    private _groupNodes = (generations: [fabric.Group][], node: Node) => {
        ...
        node.relationships &&
            generations[node.generation as number].push(
                ...node.relationships.map((relationship: Relation) => relationship.partner?._object as fabric.Group)
            )
        node.relationships &&
    ...
    private _positionNodes = () => {
        ...
        generations.forEach((generation: fabric.Group[]) => {
            ...
            generation.forEach((node: fabric.Group | undefined) => {
                node && node.set({ left })
                left += (node ? node.getBoundingRect().width : 0) + minimumDistanceBetweenNodes
            })
        })
    }
            ...
            for (const relationship of relationships) {
                if (relationship.children && relationship.children.length > 0) {
                    relationship._parentLine = this._drawParentLine(
                        ...
                        relationship.partner ? relationships.indexOf(relationship) === 0 : undefined
                    )
                    this.canvas.add(relationship._parentLine)
                    for (const child of relationship.children) {
                        this._drawChildRelations(child)
                        this._drawChildLine(child as Node, relationship._parentLine as fabric.Line)
                    }
                }
            }
        }
    ...
    private defautOnClick = (node?: Node) => {

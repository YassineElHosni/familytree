...
export default function Member({member, members}) {
    ...
    function addKid(target){
        if(!selectedKids.find(o=>o===target.id))
        setSelectedKids(o=>[...o,target.id])
    }
    ...
    useEffect(()=>{
        setDisplayedMembers(
            searchTarget.trim()==="" ? members :
            members.filter(o=>o.firstname.includes(searchTarget)
    ...
    return (
        <>
            <div className="myMember row">
                <div className="col">
                    {`${member.firstname} ${member.lastname} ${member.birthday} ch:${member.children ? member.children.length : 0}`}
                </div>
                <div className="col">
                ...
                <Form onSubmit={handleSubmit}>
                    <Modal.Body>
                        ...
                        <Form.Group>
                            ...
                            <Form.Control as="select" value={spouse} onChange={e=>setSpouse(e.target.value)}>
                                ...
                                {members ? members.map(o=>
                                    <option key={o.id} value={o.id}>{o.firstname+" "+o.lastname+" "+o.birthday}</option>
                                    ) : ""}
                            </Form.Control>
                        </Form.Group>
                        <Form.Group>
                        ...
                        <div className="selectedKids">
                            {selectedKids ? selectedKids.map(id=>{
                                let o = members.find(b=>b.id===id)
                                return <div key={o.id}
                                        id={o.id}
                                        onClick={e=>removeKid(e.target)}
                                    >
                        ...
                        <SearchInput onChange={e=>setSearchTarget(e.target.value)} placeholder="find selectedKids"/>
                        <div className="searchResults">
                            {searchResults ? searchResults.map(o=>
                                <div key={o.id}
                                    id={o.id}
